...
function iconv#cp932ext#new()
  return s:cp932ext.new()
endfunction

let s:cp932ext = {}

function s:cp932ext.new()
  let obj = copy(self)
  call obj.__init__()
  return obj
endfunction

function s:cp932ext.__init__()
  " pass
endfunction

function s:cp932ext.mbtowc(mb)
  let c1 = a:mb[0]
  if (c1 == 0x87) || (c1 >= 0xed && c1 <= 0xee) || (c1 >= 0xfa && c1 <= 0xfc)
    if len(a:mb) >= 2
      let c2 = a:mb[1]
      if (c2 >= 0x40 && c2 < 0x7f) || (c2 >= 0x80 && c2 < 0xfd)
        let i = 188 * (c1 - (c1 >= 0xe0 ? 0xc1 : 0x81)) + (c2 - (c2 >= 0x80 ? 0x41 : 0x40))
        let wc = 0xfffd
        if i < 8272
          if i < 1220
            let wc = self.cp932ext_2uni_page87[i-1128]
          endif
        elseif i < 10716
          if i < 8648
            let wc = self.cp932ext_2uni_pageed[i-8272]
          endif
        else
          if i < 11104
            let wc = self.cp932ext_2uni_pagefa[i-10716]
          endif
        endif
        if wc != 0xfffd
          return [2, [wc]]
...
function s:cp932ext.wctomb(wc)
  let summary = []
  if a:wc >= 0x2100 && a:wc < 0x22c0
    let summary = self.cp932ext_uni2indx_page21[(a:wc/0x10)-0x210]
  elseif a:wc >= 0x2400 && a:wc < 0x2480
    let summary = self.cp932ext_uni2indx_page24[(a:wc/0x10)-0x240]
  elseif a:wc >= 0x3000 && a:wc < 0x3020
    let summary = self.cp932ext_uni2indx_page30[(a:wc/0x10)-0x300]
  elseif a:wc >= 0x3200 && a:wc < 0x33d0
    let summary = self.cp932ext_uni2indx_page32[(a:wc/0x10)-0x320]
  elseif a:wc >= 0x4e00 && a:wc < 0x5590
    let summary = self.cp932ext_uni2indx_page4e[(a:wc/0x10)-0x4e0]
  elseif a:wc >= 0x5700 && a:wc < 0x59c0
    let summary = self.cp932ext_uni2indx_page57[(a:wc/0x10)-0x570]
  elseif a:wc >= 0x5b00 && a:wc < 0x5de0
    let summary = self.cp932ext_uni2indx_page5b[(a:wc/0x10)-0x5b0]
  elseif a:wc >= 0x5f00 && a:wc < 0x7ba0
    let summary = self.cp932ext_uni2indx_page5f[(a:wc/0x10)-0x5f0]
  elseif a:wc >= 0x7d00 && a:wc < 0x7fb0
    let summary = self.cp932ext_uni2indx_page7d[(a:wc/0x10)-0x7d0]
  elseif a:wc >= 0x8300 && a:wc < 0x85c0
    let summary = self.cp932ext_uni2indx_page83[(a:wc/0x10)-0x830]
  elseif a:wc >= 0x8800 && a:wc < 0x8ed0
    let summary = self.cp932ext_uni2indx_page88[(a:wc/0x10)-0x880]
  elseif a:wc >= 0x9000 && a:wc < 0x9ee0
    let summary = self.cp932ext_uni2indx_page90[(a:wc/0x10)-0x900]
  elseif a:wc >= 0xf900 && a:wc < 0xfa30
    let summary = self.cp932ext_uni2indx_pagef9[(a:wc/0x10)-0xf90]
  elseif a:wc >= 0xff00 && a:wc < 0xfff0
    let summary = self.cp932ext_uni2indx_pageff[(a:wc/0x10)-0xff0]
  endif
  if !empty(summary)
    let idx_indx = 0
    let idx_used = 1
    ...
    if bitwise#and(used, bitwise#lshift(1, i))
      ...
      let c = self.cp932ext_2charset[summary[idx_indx] + used]
      return [c / 0x100, c % 0x100]
    endif
  endif
  throw 'ICONV: RET_ILUNI'
endfunction

function s:cp932ext.flush()
  return []
endfunction

let s:cp932ext.cp932ext_2uni_page87 = [
\
...
let s:cp932ext.cp932ext_2uni_pageed = [
\
...
let s:cp932ext.cp932ext_2uni_pagefa = [
\
...
let s:cp932ext.cp932ext_2charset = [
...
let s:cp932ext.cp932ext_uni2indx_page21 = [
\
...
let s:cp932ext.cp932ext_uni2indx_page24 = [
\
\ [   35, 0x0000 ], [   35, 0x0000 ], [   35, 0x0000 ], [   35, 0x0000 ],
\ [   35, 0x0000 ], [   35, 0x0000 ], [   35, 0xffff ], [   51, 0x000f ],
\]

let s:cp932ext.cp932ext_uni2indx_page30 = [
\
\ [   55, 0x0000 ], [   55, 0xa000 ],
\]

let s:cp932ext.cp932ext_uni2indx_page32 = [
\
...
let s:cp932ext.cp932ext_uni2indx_page4e = [
\
...
let s:cp932ext.cp932ext_uni2indx_page57 = [
\
...
let s:cp932ext.cp932ext_uni2indx_page5b = [
\
...
let s:cp932ext.cp932ext_uni2indx_page5f = [
\
...
let s:cp932ext.cp932ext_uni2indx_page7d = [
\
...
let s:cp932ext.cp932ext_uni2indx_page83 = [
\
...
let s:cp932ext.cp932ext_uni2indx_page88 = [
\
...
let s:cp932ext.cp932ext_uni2indx_page90 = [
\
...
let s:cp932ext.cp932ext_uni2indx_pagef9 = [
\
...
let s:cp932ext.cp932ext_uni2indx_pageff = [
\
